...
nn = MLPClassifier(random_state=1, max_iter=400, beta_1 = 0.888, beta_2 = 0.99).fit(train, np.ravel(target))
nn_predict = nn.predict(test)

print('Accuracy: ' + str(metrics.accuracy_score(target_test, nn_predict)))
print('Precision: ' + str(metrics.precision_score(target_test, nn_predict)))
print('Recall: ' + str(metrics.recall_score(target_test, nn_predict)))
print('F1 Score: ' + str(metrics.f1_score(target_test, nn_predict)))

print('Confusion Matrix:')
draw_confusion_matrix(target_test, nn_predict, ["Not Obfuscated", "Obfuscated"])
